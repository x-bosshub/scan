...
# ==========================================
# โหลดไลบรารี YOLOv8 (Object Detection & Tracking)
# ==========================================
try:
    from ultralytics import YOLO
    yolo_model = YOLO('yolov8n.pt') 
    yolo_available = True
    print("[SYSTEM] โหลดไลบรารี YOLOv8 สำเร็จ")
except ImportError:
    yolo_available = False
    print("[WARNING] ไม่พบไลบรารี ultralytics กรุณาติดตั้ง (pip install ultralytics)")
...
# ==========================================
# AI: TRACKING, SPEED ESTIMATION & FACE RECOGNITION
# ==========================================
track_history = {} 
PIXEL_TO_METER = 0.03 # ปรับค่านี้เพื่อให้ความเร็วตรงกับความกว้างถนนจริง

def process_ai_tracking(frame):
    global last_alert_time, telegram_settings, track_history
    
    if not yolo_available: return frame

    # สั่ง YOLO รันแบบ Tracking
    results = yolo_model.track(frame, classes=[0, 2, 3, 5, 7], conf=0.5, persist=True, verbose=False, imgsz=320)
    ...
    for r in results:
        ...
        for box, track_id, cls, conf in zip(boxes.xyxy, track_ids, boxes.cls, boxes.conf):
            ...
            # เก็บประวัติการเดินทางเพื่อคำนวณความเร็ว
            ...
            if len(track_history[track_id]) >= 5: 
                ...
                if dt > 0:
                    ...
                    speed_mps = (dist_px * PIXEL_TO_METER) / dt
            ...
            if cls_id == 0:
                ...
                # ครอปภาพคนเพื่อส่งเข้า Face Recognition
                if face_rec_available and known_face_encodings:
                    try:
                        pad = 10
                        h, w = frame.shape[:2]
                ...
                if person_name != "Unknown":
                    display_name = f"{person_name} (ID:{track_id})"
                    color = (0, 255, 0) 
                    label_th = f"คุณ{person_name}"
                else:
                    display_name = f"Person (ID:{track_id})"
                    
            # ถ้าเป็นยานพาหนะ
            else:
                color = (255, 0, 0) 
                display_name = f"Vehicle (ID:{track_id})"
                label_th = "ยานพาหนะ"

            info_text = f"{display_name} {int(speed_kmh)}km/h"

            # วาดกรอบบนภาพ
            cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
            cv2.rectangle(frame, (x1, y1 - 25), (x2, y1), color, cv2.FILLED)
            cv2.putText(frame, info_text, (x1 + 4, y1 - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
            
            # เตรียมข้อความส่ง Telegram
            if speed_kmh > 0:
                alerts_this_frame.append(f"[{label_th} ID:{track_id}] ความเร็ว ~{int(speed_kmh)} km/h")
            else:
                alerts_this_frame.append(f"[{label_th} ID:{track_id}] กำลังเคลื่อนไหว")

    # สั่งให้บอทส่งข้อความเข้า Telegram
    if len(alerts_this_frame) > 0 and telegram_settings["enabled"]:
        current_time = time.time()
        if current_time - last_alert_time > telegram_settings["cooldown"]:
            msg_details = "\n- ".join(alerts_this_frame)
            msg = f"🚨 AI ตรวจพบความเคลื่อนไหว:\n- {msg_details}"
...
def generate_frames(rtsp_url):
    ...
    try:
        while True:
            ...
            frame = process_ai_tracking(frame)
